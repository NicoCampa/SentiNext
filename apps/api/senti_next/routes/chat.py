"""Chat endpoints: /chat, /chat/simple, /chat/*, /support/*."""
...
@router.get("/support/thread", response_model=List[SupportMessage])
def get_support_thread(mark_read: bool = True) -> List[SupportMessage]:
    user_id = "local"
    try:
...
@router.post("/support/message", response_model=SupportMessage)
def send_support_message(request: SupportMessageRequest) -> SupportMessage:
    user_id = "local"
    message = (request.message or "").strip()
    if not message:
        raise HTTPException(status_code=400, detail="Message cannot be empty.")
...
@router.get("/support/unread", response_model=UnreadCountResponse)
def get_support_unread_count() -> UnreadCountResponse:
    user_id = "local"
    try:
...
# ---------------------------------------------------------------------------
# Admin chat/support endpoints
# ---------------------------------------------------------------------------

@router.get("/admin/support/threads", response_model=List[SupportThreadSummary])
def get_admin_support_threads(limit: int = 100) -> List[SupportThreadSummary]:
    try:
...
@router.get("/admin/support/thread/{thread_user_id}", response_model=List[SupportMessage])
def get_admin_support_thread(thread_user_id: str, mark_read: bool = True) -> List[SupportMessage]:
    try:
...
@router.post("/admin/support/thread/{thread_user_id}/reply", response_model=SupportMessage)
def send_admin_support_reply(thread_user_id: str, request: SupportMessageRequest) -> SupportMessage:
    message = (request.message or "").strip()
    if not message:
        raise HTTPException(status_code=400, detail="Message cannot be empty.")
    sender_id = "admin"
...
@router.get("/admin/support/unread", response_model=UnreadCountResponse)
def get_admin_support_unread_count() -> UnreadCountResponse:
    try:
...
@router.get("/admin/chat-sessions", response_model=List[AdminChatSession])
def get_admin_chat_sessions(limit: int = 100) -> List[AdminChatSession]:
    try:
...
@router.get("/admin/chat-history/{session_id}", response_model=List[ChatMessage])
def get_admin_chat_history(session_id: str) -> List[ChatMessage]:
    try:
