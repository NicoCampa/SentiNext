...
// Valid-format dummy key for CI/build (decodes to "ci.build.local$").
// Clerk validates key format at init; this lets SSG succeed without a real key.
const DUMMY_CLERK_KEY = "pk_test_Y2kuYnVpbGQubG9jYWwk";

export function RootProviders({ children }: { children: ReactNode }) {
  const envKey = process.env.NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY;
  const hasClerkKey = !!envKey;
  const publishableKey = envKey || DUMMY_CLERK_KEY;

  return (
    <ClerkProvider dynamic publishableKey={publishableKey} appearance={clerkAppearance}>
      {hasClerkKey ? (
        <>
          <ClerkTokenProvider />
          <AuthGate>
