...
class RunConfig:
    ...
    timeout_s: int = 60
    retries: int = 2
    cache_db: Path = Path("benchmarks/.cache/cache.sqlite")
...
def load_config(path: Path) -> BenchmarkConfig:
    ...
    # Resolve relative paths from the repo root (more intuitive when configs live under benchmarks/configs/).
    ...
    run = RunConfig(
        ...
        cache_db=_as_path(run_raw.get("cache_db", "benchmarks/.cache/cache.sqlite"), base_dir=base_dir),
        results_dir=_as_path(run_raw.get("results_dir", "benchmarks/results"), base_dir=base_dir),
